...
The below is an example to split pcap files into 10 files.

    slicecap.py -r source.pcap -n 10 -- "cat > dest-{FRAG_ID}.pcap"

The `-r` or `--infile` option specifies input pcap file to slice.  Stdin is not
acceptable because `slicecap` will change file handle pointer using
the `seek()` method.
...
the replacement will be done by Python formatting function, you can specify formatting rules using the standard Python text formatter.
